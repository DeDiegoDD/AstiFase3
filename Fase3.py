"""-----------------------------------------
Desafio programacion ASTI ROBOTICS 2022/2023
...
class Robot:
    def __init__(self, Tfin):
        ...
        self.__posSensor = [self.__x + (self.__l / 2) * np.cos(self.__ang),
                            self.__y + (self.__l / 2) * np.sin(self.__ang)]

        self.__rectas = list([[0, 10], [0, 10], [0, 10], [0, 10], [0, 10], [1, 0], [0, 10], [0, 10], [1, 0]])
        self.__destinos = list(
            [[20, 10], [20, 10], [20, 10], [20, 10], [20, 10], [20, 20], [20, 10], [20, 10], [20, 20]])
        self.__origenes = list(
            [[0, 10, 0], [0, 10, np.pi / 4], [0, 20, 0], [0, 0, np.pi / 4], [0, 0, -np.pi / 4], [0, 10, -np.pi / 4],
             [0, 10, np.pi], [0, 20, np.pi], [0, 10, 3 * np.pi / 4]])
    ...
    def actTiempo(self):
        ...
        posEsquinaDer = [self.__posSensor[0] + (self.__l / 2) * np.cos(self.__ang - np.pi / 2),
                         self.__posSensor[1] + (self.__l / 2) * np.sin(self.__ang - np.pi / 2)]
        posEsquinaIzq = [self.__posSensor[0] + (self.__l / 2) * np.cos(self.__ang + np.pi / 2),
                         self.__posSensor[1] + (self.__l / 2) * np.sin(self.__ang + np.pi / 2)]
    ...
    def pintar(self):
        ...
        for i in range(n):
            ...
            if self.__rectas[i][0] is np.nan or np.isinf(self.__rectas[i][0]):
                y = np.linspace(np.amin([np.amin(self.__ylist[i]), self.__destinos[i][1]]),
                                np.amax([np.amax(self.__ylist[i]), self.__destinos[i][1]]), 1000)
                pl.plot([self.__rectas[i][1]] * 1000, y, color="blue", linewidth=1.0, linestyle="-",
                        label='Linea de guia')
            else:
                x = np.linspace(np.amin([np.amin(self.__xlist[i]), self.__destinos[i][0]]),
                                np.amax([np.amax(self.__xlist[i]), self.__destinos[i][0]]), 1000)
                pl.plot(x, x * self.__rectas[i][0] + self.__rectas[i][1], color="blue", linewidth=1.0, linestyle="-",
                        label='Linea de guia')
            pl.hold(True)
            # pl.plot(self.__xlist[i], self.__ylist[i], color="blue", linewidth=1.0, linestyle="-")
            pl.quiver(self.__xlist[i], self.__ylist[i], np.cos(self.__anglist[i]), np.sin(self.__anglist[i]),
                      label='Robot')
